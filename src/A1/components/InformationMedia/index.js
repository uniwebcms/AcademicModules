        ...
        <Container>
            <div
                ...
                <div
                    ...
                    {pretitle ? (
                        <h3 className='mb-3 font-medium  text-lg md:text-xl lg:text-2xl'>{stripTags(pretitle)}</h3>
                    ) : null}
                    <h2 className='text-2xl font-bold tracking-tight  md:text-3xl lg:text-4xl'>{stripTags(title)}</h2>
                    {subtitle ? (
                    ...
                    {paragraphs?.length ? (
                        <SafeHtml className='mt-6 text-sm md:text-base lg:text-lg' value={paragraphs} />
                    ) : null}
                </div>
                <div
                    className={twJoin(
    ...
    return (
        <section ref={containerRef} className={twJoin('py-24 sm:py-32 relative')}>
            <div className='mx-auto lg:max-w-7xl px-6 lg:px-8 text-center'>
                {pretitle ? (
                    <h3 className='mb-3 font-medium  text-xl md:text-2xl lg:text-3xl'>{stripTags(pretitle)}</h3>
                ) : null}
                <h2 className='text-2xl font-bold tracking-tight md:text-3xl lg:text-4xl'>{stripTags(title)}</h2>
                {subtitle ? (
                ...
                <div className='relative overflow-hidden pt-16 lg:pt-20'>
                    <div className='mx-auto max-w-7xl px-6 lg:px-8'>
                        ...
                        <div className='relative' aria-hidden='true'>
                            <div
                                style={gradient}
                                className={twJoin('absolute -inset-x-20 bottom-0 bg-gradient-to-t pt-[7%]')}
                            />
                        </div>
                    </div>
                </div>
