...
import { twMerge, twJoin, stripTags, getPageProfile } from '@uniwebcms/module-sdk';
        ...
        <Container className="flex flex-col items-center justify-center">
            ...
            {main.banner && (
                <div className="absolute inset-0 w-full h-full z-0">
                    ...
                    />
                    <div
                        className="absolute inset-0 bg-text-color-10 mix-blend-multiply"
                        aria-hidden="true"
                    />
                </div>
            )}
            <div className="z-10 text-center mx-auto max-w-3xl">
                {pretitle && (
                    <h3 className="mb-3 text-xl font-light md:text-2xl lg:text-3xl">
                        {stripTags(pretitle)}
                    </h3>
                )}
                <h2 className="text-3xl font-bold  md:text-4xl lg:text-5xl">{stripTags(title)}</h2>
                {subtitle && (
                    <p className="mt-4 text-base md:text-lg lg:text-xl text-text-color-80">
                        {stripTags(subtitle)}
                    </p>
                )}
            </div>
                                    ...
                                    >
                                        <span className="flex-shrink-0">
                                            <div className="flex items-center justify-center w-10 h-10 border-2 rounded-full border-text-color-70">
                                                <div className="text-text-color-80">
                                                    {index + 1}
                                                </div>
                                            </div>
                                        </span>
