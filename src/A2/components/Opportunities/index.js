            ...
            <div className="w-full px-4 xs:px-6 lg:px-8 py-8 md:py-12 lg:py-16">
                ...
                >
                    {filteredItems.map((item) => (
                        <OpportunityCard key={item.key} data={item} />
                    ))}
                </div>
...
const OpportunityCard = ({ data }) => {
    return (
        <Link
            to={data.to}
            className={twJoin(
                'group p-5 flex justify-start gap-6 bg-text-color-0 rounded-[var(--border-radius)] border border-text-color/20 hover:border-primary-300 hover:shadow-md transition-all cursor-pointer'
            )}
        >
            <div
                className={twJoin(
                    'w-32 h-32 flex-shrink-0 rounded-[var(--border-radius)] overflow-hidden hidden md:block'
                )}
            >
                <Image
                    profile={data.p}
        ...
        <div className="flex flex-col items-center justify-center min-h-[600px] text-center p-4">
            ...
            <p className="mt-4 text-text-color text-lg lg:text-2xl">
                {block.website.localize({
                    en: 'Loading opportunities...',
                    fr: 'Chargement des opportunités...',
                })}
            </p>
