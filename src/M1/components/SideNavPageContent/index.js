    ...
    return (
        <section className="mb-12">
            ...
            <div className="space-y-6">
                {items.map((item, index) => {
                    const { pretitle, title, subtitle } = item;

                    return (
                        <div key={index} className="flex gap-6 p-4 bg-neutral-50 rounded-lg">
                            <div className="text-sm text-neutral-500 whitespace-nowrap mt-0.5 w-32">
                                {pretitle}
                            </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-8">
                        {logoItem && (
                            <div>
                                ...
                                {logoItem.links[0] && (
                                    <Link
                                        to={logoItem.links[0].href}
                                        className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium bg-text-color text-text-color-0 hover:bg-text-color/80 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 h-10 px-4 py-2 w-full mb-4"
                                    >
                                        <LuDownload className="w-4 h-4 mr-2 text-inherit" />
                                        {logoItem.links[0].label}
                                    </Link>
                                )}
                                <ul className="text-sm space-y-2 text-neutral-600 list-disc pl-4 marker:text-text-color/40">
...
const Contact = ({ block }) => {
    ...
    const resourceItemLinks =
        resourceItem?.links?.map((link, index) => {
            return {
                label: link.label,
                href: link.href,
            ...
            <div className="rounded-lg border shadow-sm">
                <div className="p-6 space-y-8">
                    ...
                    <div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4">
                            {resourceItemLinks.map((link, idx) => (
                                <Link
                                    to={link.href}
                                    key={idx}
                                    className="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm font-medium bg-text-color text-text-color-0 hover:bg-text-color/80 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 border h-10 px-4 py-2 w-full justify-start btn-secondary"
                                >
                                    {link.icon && (
                                        <Icon
                                            icon={link.icon}
...
export default function SideNavPageContent(props) {
    const { block } = props;

    const { title, subtitle } = block.getBlockContent();

    const { display = 'simple_card' } = block.getBlockProperties();

    if (display === 'simple_card') {
        return <SimpleCard block={block} />;
    } else if (display === 'timeline_card') {
        return <TimeLineCard block={block} />;
    } else if (display === 'paragraphs') {
        return <Paragraphs block={block} />;
    } else if (display === 'branding_showcase') {
        return <BrandingShowcase block={block} />;
    } else if (display === 'contact_resources') {
