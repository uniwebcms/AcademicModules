...
class ScrollspyNav extends Component {
    ...
    onScroll() {
        ...
        this.props.scrollTargetIds.forEach((sectionID, index) => {
            ...
            // let top = this.scroller
            //     ? this.getSectionDom(sectionID).offsetTop -
            //       (this.headerBackground
            //           ? document.querySelector("nav[data-nav='list']").scrollHeight
            //           : 0) +
            //       this.offset
            ...
            scrollSectionOffsetTop =
                top -
                (this.headerBackground
                    ? document.querySelector("nav[data-nav='list']")
                          .scrollHeight
                    : 0);

            if (this.scroller) {
                if (
                    scrollSectionOffsetTop + this.offset <=
                    this.scroller.getBoundingClientRect().top - this.offset
                ) {
                ...
                if (
                    ...
                    window.pageYOffset - this.offset >=
                        scrollSectionOffsetTop &&
                    window.pageYOffset <
                        scrollSectionOffsetTop + section.scrollHeight
                ) {
                    this.getNavLinkElement(sectionID).classList.add(
                        this.activeNavClass
                    );
                ...
                if (
                    ...
                    index === this.props.scrollTargetIds.length - 1
                ) {
                    this.getNavLinkElement(sectionID).classList.add(
                        this.activeNavClass
                    );
    ...
    componentDidUpdate(preProps) {
        if (
            JSON.stringify(preProps?.scrollTargetIds) !==
            JSON.stringify(this.props.scrollTargetIds)
        ) {
            if (this.scroller)
