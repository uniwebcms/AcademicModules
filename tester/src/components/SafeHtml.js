...
import Cite from './CiteRenderer';

function CiteRender({ data, style = 'apa', locale = 'en-US' }) {
    ...
    let output = cite.format('bibliography', {
        ...
        template: style, // 'vancouver' | 'apa' | 'harvard | 'chicago', 'mla'
        lang: locale,
        nosort: true,
    });
            ...
            if (domNode.attribs && domNode.name === 'a') {
                ...
                return (
                    <React.Suspense fallback={null}>
                        <CiteRender {...props} data={data ? JSON.parse(data) : ''}></CiteRender>
                    </React.Suspense>
                );
            }
